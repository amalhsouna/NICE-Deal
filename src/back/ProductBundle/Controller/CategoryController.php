...
class CategoryController extends Controller
{
    ...
    public function putCategoryAction($id, Request $request)
    {
        $message = '';
        $form = $this->get('back_product.category.form');
        $formHandler = $this->get('back_product.handler.update.category');

        $processForm = $formHandler->process($id, $request);

        if ($processForm === true) {
            $this->get('session')->getFlashBag()->add('success', 'account.update_password.success.message');
            $message = 'modification effectuée avec succée';
        }

        return $this->render('backProductBundle:Categories:editCategory.html.twig', array('form' => $form->createView(), 'message' => $message));
    }
}
